...
var FBXLoader = ( function () {
		...
		parseProperty: function ( reader ) {
			...
			switch ( type ) {
				...
				case 'l':
					...
					let decompressed = inflateSync( new Uint8Array( reader.getArrayBuffer( compressedLength ) ) ); // eslint-disable-line no-undef
					var reader2 = new BinaryReader( decompressed.buffer );
	...
	function isFbxFormatBinary( buffer ) {
		...
		var buffer_header = convertArrayBufferToString(buffer, 0, CORRECT.length);

		return buffer.byteLength >= CORRECT.length && CORRECT === buffer_header;
	...
	function convertArrayBufferToString( buffer, from, to ) {
		from = from || 0;
		to = to || buffer.byteLength;
		let buffer_array = new Uint8Array(buffer, from ,to - from);

		return textDecoder.decode(buffer_array);
